...
/**
 * Shown signals, newest first. Vendor filtering matches the denormalized enriched
 * `vendors` JSON (a `LIKE` on the array text) or the source's tracked vendor, so an item
 * about a competitor surfaces on that competitor's page even from a neutral source.
 */
